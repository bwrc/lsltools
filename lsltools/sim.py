...
class MarkerData(threading.Thread):
    ...
    def __init__(self,stream_name="MARKER",stream_type="MRK",srate=[1,4],
        ...
        self.interval = 1.0 / float(random.randint(self.srate[0],self.srate[1]))
        self.last_push = pylsl.local_clock()
        self.running = True
    ...
    def set_markers(self,markers,apppend=False):
...
class ECGData(threading.Thread):
    ...
    """
    def __init__(self,ecgsynpath,O="/tmp/dump.dat",n=256,s=256,S=256,a=0,h=60,
                 H=1,f=0.1,F=0.25,v=0.01,V=0.01,q=0.5,R=1,stream_name="ECGSYN"):
    ...
    def generate_data(self,append_flag):
        ...
        status = subprocess.call([self.path,"-O",self.O,"-n",self.n,
...
class EEGData(threading.Thread):
    ...
    """
    def __init__(self,data=False,ar_coefs=False,p=20,p2p=80,nch=1,srate=512,
                 stream_name='EEGSYN'):
    ...
    def generate_data(self,append_flag):
        ...
        if self.osc:
            for o in self.osc:
                ...
                s*=(self.osc[o][-1]*numpy.max(abs(new_data)))/numpy.max(abs(s))
                new_data+=s
